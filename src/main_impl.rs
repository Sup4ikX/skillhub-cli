...
use crate::registry::{GITHUB_API, RegistryClient, SkillRef};
...
fn dispatch(cli: Cli) -> anyhow::Result<()> {
    match cli.effective_command() {
        ...
        Commands::Help => {
            ...
            let n = crate::Installer::new(crate::Config::load()?)
                .list_installed()?
                .len();
            if n > 0 {
                println!("\nInstalled skills: {}", n);
        ...
        cmd => {
            ...
            let mut installer = crate::Installer::new(config);
            ...
            match cmd {
                ...
                Commands::Check { target } => cmd_check(&client, &installer, target, cli.json),
                Commands::Audit { target } => cmd_audit(&client, &installer, target, cli.json),
            }
        }
    }
}
...
fn cmd_agents(json: bool) -> anyhow::Result<()> {
    ...
    if json {
        let agents: Vec<serde_json::Value> = crate::agent::AgentKind::all()
            .iter()
            .map(|a| {
    ...
    for a in crate::agent::AgentKind::all() {
...
fn cmd_install(
    ...
    let skill = match client.load_cache() {
        Ok(r) => r
            ...
            .ok_or_else(|| SkillHubError::SkillNotFound(name.to_string()))?,
        Err(_) => {
    ...
    let downloaded = match installer.download_skill(&skill.download_url) {
        Ok(c) if !c.is_empty() => c,
        Ok(_) => {
            if !no_scan {
                println!("{}", "security scan: skipped (empty)".dimmed());
            }
            String::new()
        }
        Err(e) => {
    ...
    if !no_scan && !downloaded.is_empty() {
        let scan_result = crate::scanner::scan(&downloaded);
        if scan_result.passed {
            ...
            for f in &scan_result.findings {
                let sev = match f.severity {
                    crate::scanner::Severity::High => "HIGH".red(),
                    crate::scanner::Severity::Medium => "MED".yellow(),
                    crate::scanner::Severity::Low => "low".dimmed(),
                };
                println!(
                    "  [{}] {} (line {}): {}",
                    sev,
    ...
    if project {
        ...
        let body = if !downloaded.is_empty() {
            downloaded.clone()
        } else {
            format!("# {}\n\n{}", skill.name, skill.description)
        };

        let skill_ref = format!("\n## {} (v{})\n\n{}", skill.name, skill.version, body);
        ...
        let paths = installer.install(&skill, &downloaded)?;
        for p in &paths {
    ...
    if !downloaded.is_empty() {
        let agents: Vec<String> = skill.compatibility.agents.clone();
        let report = crate::audit::audit(&skill.name, &downloaded, &agents);

        let report_dir = installer.config.cache_dir.join("audits");
        let safe = skill.name.replace('@', "").replace('/', "_");
        let report_file = report_dir.join(format!("{}.json", safe));
        if let Err(e) = std::fs::create_dir_all(&report_dir) {
            if !json {
                eprintln!("warn: cannot create audit dir: {}", e);
            }
        } else if let Ok(s) = serde_json::to_string_pretty(&report) {
            if let Err(e) = std::fs::write(&report_file, s) {
                if !json {
                    eprintln!("warn: cannot write audit: {}", e);
                }
            }
        }
        ...
        let verdict_colored = match report.verdict {
            crate::audit::Verdict::Pass => "PASS".green(),
            crate::audit::Verdict::Warn => "WARN".yellow(),
            crate::audit::Verdict::Fail => "FAIL".red(),
        };
        println!(
            "  Audit:  security {:>3}   quality {:>3}   compat {:>3}   overall {:>3}  [{}]",
        ...
        if !report.security.findings.is_empty() {
            ...
            for f in report.security.findings.iter().take(5) {
                println!("    [{}] {} (line {})", f.severity, f.kind, f.line);
            }
        }
        if !report.quality.suggestions.is_empty() {
            println!("  Quality suggestions:");
            for s in report.quality.suggestions.iter().take(3) {
                println!("    - {}", s.dimmed());
            }
        }
        println!(
            "  Full report: {}",
            format!("skillhub check {}", skill.name).dimmed()
        );
    }
...
fn cmd_upgrade(
    ...
    let mut to_upgrade: Vec<PendingUpgrade> = Vec::new();
    ...
    for (_, skill) in &installed {
        if let Some(reg) = registry.skills.iter().find(|s| s.name == skill.name) {
            if version_gt(&reg.version, &skill.version) {
                to_upgrade.push(PendingUpgrade {
                    name: skill.name.clone(),
                    old_v: skill.version.clone(),
                    new_v: reg.version.clone(),
                });
            }
        }
    }
    ...
    if dry_run {
        if json {
            println!(
                "{}",
                serde_json::json!({
                    "to_upgrade": to_upgrade.iter().map(|u| {
                        serde_json::json!({
                            "name": u.name,
                            "old_version": u.old_v,
                            "new_version": u.new_v
    ...
    for u in &to_upgrade {
        if !all {
            let ok = dialoguer::Confirm::new()
                .with_prompt(format!("Upgrade {} v{} → v{}?", u.name, u.old_v, u.new_v))
                .default(true)
                .interact()?;
            if !ok {
                continue;
        ...
        let ref_ = SkillRef::parse(&u.name)?;
        let reg_skill = match registry.skills.iter().find(|s| ref_.matches(s)) {
            Some(s) => s.clone(),
            None => {
                if json {
                    println!("{}", json_err(&format!("skill not found: {}", u.name)));
                } else {
                    eprintln!("skill not found: {}", u.name);
                }
                continue;
            }
        };
...
fn resolve_target(
    ...
    if path.exists() && path.is_file() {
        let content = std::fs::read_to_string(path).map_err(SkillHubError::Io)?;
        let name = path
            .file_stem()
            .and_then(|s| s.to_str())
            .map(|s| format!("@local/{}", s))
            .unwrap_or_else(|| "@local/skill".to_string());
        return Ok((name, content, Vec::new()));
    }

    if path.exists() && path.is_dir() {
        let skill_md = path.join("SKILL.md");
        if skill_md.exists() {
            let content = std::fs::read_to_string(&skill_md).map_err(SkillHubError::Io)?;
            let name = path
                .file_name()
                .and_then(|s| s.to_str())
                .map(|s| format!("@local/{}", s))
    ...
    let ref_ = SkillRef::parse(target)?;
    if let Some((_, skill)) = installer
        .list_installed()?
        .into_iter()
        .find(|(_, s)| ref_.matches(s))
    {
        let dir = installer.skill_path(&ref_)?;
        let content = std::fs::read_to_string(dir.join("SKILL.md")).map_err(SkillHubError::Io)?;
        let agents = skill.compatibility.agents.clone();
        return Ok((skill.name, content, agents));
    }

    let skill = client.find_skill(&ref_)?;
    let content = installer.download_skill(&skill.download_url)?;
    let agents = skill.compatibility.agents.clone();
    Ok((skill.name, content, agents))
}

fn print_audit_text(report: &crate::audit::AuditReport) {
    use colored::Colorize;

    println!("{}", format!("Audit: {}", report.name).cyan().bold());
    println!();
    ...
    println!(
        "  Overall: {}/100  [{}]",
        report.overall_score.to_string().bold(),
        verdict_colored.bold()
    );
    println!();
    ...
    println!();
    println!("  {:<18} {:>4}/100", "Quality".bold(), report.quality.score);
    let q = &report.quality;
    println!(
        "    title: {}  description: {}  sections: {}  code blocks: {}",
        if q.title_present {
            "yes".green()
        } else {
            "no".red()
        },
        if q.description_present {
            "yes".green()
        } else {
            "no".red()
        },
    ...
    println!(
        "    example: {}  list items: {}  frontmatter: {}",
        if q.example_present {
            "yes".green()
        } else {
            "no".red()
        },
        q.list_item_count,
        if q.frontmatter_valid {
            "valid".green()
        } else if q.frontmatter_present {
            "incomplete".yellow()
        } else {
            "missing".red()
        }
    );
    println!("    chars: {}  words: {}", q.char_count, q.word_count);
    ...
    if report.compatibility.per_agent.is_empty() {
        println!(
            "    {}",
            "no compatibility data (registry did not list agents)".dimmed()
        );
    } else {
        for a in &report.compatibility.per_agent {
            let status = match a.status.as_str() {
                "supported" => a.status.green(),
                "partial" => a.status.yellow(),
                _ => a.status.red(),
            };
            println!("    {:<14} {}  {:>3}/100", a.agent, status, a.score);
            for note in &a.notes {
                println!("      - {}", note.dimmed());
            }
        }
...
fn cmd_check(
    ...
    let (name, content, agents) = resolve_target(client, installer, target)?;
    let report = crate::audit::audit(&name, &content, &agents);
...
fn cmd_audit(
    ...
    let report_path = installer.config.cache_dir.join("audits");
    let safe_name = name.replace('@', "").replace('/', "_");
    let report_file = report_path.join(format!("{}.json", safe_name));
    ...
    if json {
        ...
        print_audit_text(&report);
        println!();
        println!("{}", format!("Report saved to {:?}", report_file).dimmed());
    }
